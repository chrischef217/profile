...
async function probe(name:string,url:string,options:RequestInit={}){const started=Date.now();try{const response=await fetch(url,{...options,signal:AbortSignal.timeout(30000)});const contentType=response.headers.get('content-type')||'';const text=await response.text();let body:any=null;try{body=JSON.parse(text)}catch{body=text.slice(0,500)}return{name,ok:response.ok,status:response.status,content_type:contentType,duration_ms:Date.now()-started,body}}catch(error){return{name,ok:false,status:0,content_type:'',duration_ms:Date.now()-started,error:error instanceof Error?error.message:String(error)}}}
...
checks.push(runnerUnauthorized,runnerAuthorized,gameHtml,gamePayload);
checks.push({name:'ASSERT_RUNNER_UNAUTHORIZED',ok:runnerUnauthorized.status===401});
checks.push({name:'ASSERT_RUNNER_AUTHORIZED',ok:runnerAuthorized.status===200});
checks.push({name:'ASSERT_GAME_HTML',ok:gameHtml.status===200&&String(gameHtml.content_type).includes('text/html')});
checks.push({name:'ASSERT_GAME_PAYLOAD',ok:gamePayload.status===200&&(String(gamePayload.content_type).includes('javascript')||String(gamePayload.body).includes('__UG_V5_PAYLOAD'))});
